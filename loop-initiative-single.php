...
if (have_posts()) :
	...
	echo "<article class='".implode(" ", get_post_class('post-entry post-entry-type-'.$post_format . " " . $post_class . " ".$with_slider))."' ".avia_markup_helper(array('context' => 'entry','echo'=>false)).">";
        ...
        echo "<div class='entry-content-wrapper clearfix {$post_format}-content'>";
		    ...
		    $sdgs = get_the_term_list($id_id, 'sdg', '', ', ','').' ';
		    $continent = get_the_term_list($id_id, 'continent', '', ', ','').' ';
		    $focusarea= get_the_term_list($id_id, 'focusarea', '', ', ','').' ';
                }
                ...
                echo "<span class='date-title'>Posted on: </span><time class='date-container minor-meta updated' $markup>".get_the_time(get_option('date_format'))."</time>";
...
// get initiative meta data
/* custom meta data for initiatives */
$meta_url = get_post_meta($post->ID,"initiativeURL",true);
$meta_datacoll = get_post_meta($post->ID,"initiativeDataCollection",true);
$meta_startdate = get_post_meta($post->ID,"initiativeStartDate",true);
$end = '';
$styled_meta = '';

if (strlen($sdgs) + strlen($focusarea) + strlen($continent) + strlen($meta_url) + strlen($meta_datacoll) + strlen($meta_startdate) > 1) {
	$styled_meta .= "<ul class='custom-meta'>";
	$end = "</ul>";
}

if(strlen($sdgs)>1) {
	$styled_meta .= '<li><span>Related UN Sustainable Development Goals:</span>'.$sdgs.'</li>';
}
if(strlen($continent)>1) {
	$styled_meta .= '<li><span>Continent:</span>'.$continent.'</li>';
}
if(strlen($focusarea)>1) {
	$styled_meta .= '<li><span>Focus Area:</span> '.$focusarea.'</li>';
}
if (strlen($meta_url)>1) {
	$styled_meta .= "<li><span>Initiative Homepage:</span> <a href=".$meta_url.">".$meta_url."</a></li>";
}
if (strlen($meta_datacoll)>1) {
	$styled_meta .= "<li><span>Types of data collection used:</span> ".$meta_datacoll."</li>";
}
if (strlen($meta_startdate)>1) {
	$styled_meta .= "<li><span>Initiative start date:</span> ".$meta_startdate."</li>";
}

$styled_meta = $styled_meta.$end;
echo $styled_meta;
